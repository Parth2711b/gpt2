...
// Run:    ./gpt2 [prompt] [tokens] [temp] [topK] [topP] [repPenalty] [--weights <path>]
//         temp 0 = greedy.  Defaults: 20 tokens, 0.8 temp, topK 40, topP 0.9, penalty 1.2.
//         --weights can appear anywhere; default path is ../weights.
//
// Weight files expected in the weights directory (txt or matching .bin):
//   transformer.wte.weight, transformer.wpe.weight, transformer.ln_f.{weight,bias}
...
// Loads a matching .bin (raw float32) if present, otherwise parses the .txt file.
tensor readValues(const std::string& path, std::size_t maxCount = 0) {
    if (endsWith(path, ".txt")) {
        ...
        if (bin) {
            ...
            tensor values(count);
            bin.seekg(0);
            bin.read(reinterpret_cast<char*>(values.data()), static_cast<std::streamsize>(bytes));
            if (!bin) throw std::runtime_error("Could not read: " + binPath);
            return values;
        }
    }

    std::ifstream stream(path);
    if (!stream) throw std::runtime_error("Could not open weights file: " + path);

    tensor values;
    if (maxCount != 0) values.reserve(maxCount);
    float value = 0.0f;
    while ((maxCount == 0 || values.size() < maxCount) && stream >> value) {
        values.push_back(value);
    }
    if (maxCount != 0) {
        if (values.size() != maxCount) throw std::runtime_error("Too few values in: " + path);
        if (stream >> value) throw std::runtime_error("Too many values (wrong shape?) in: " + path);
    }
    return values;
}

// HF Conv1D stores weights as [in][out]; transpose to [out][in] so forwardPass rows are neurons.
tensor transposeMatrix(std::span<const float> values, int rows, int cols) {
...
tensor loadMatrix(const std::string& path, int inputDim, int outputDim) {
    return transposeMatrix(readValues(path, static_cast<std::size_t>(inputDim) * outputDim),
                           inputDim, outputDim);
}

void loadTransformerLayer(TransformerInput& layer, int index, const std::string& root) {
    const std::string prefix = root + "/transformer.h." + std::to_string(index);
    constexpr std::ptrdiff_t block = static_cast<std::ptrdiff_t>(kEmbeddingDim) * kEmbeddingDim;

    // c_attn is fused: after transpose, rows are [q (768) | k (768) | v (768)]
    tensor fusedWeights = loadMatrix(prefix + ".attn.c_attn.weight.txt", kEmbeddingDim, 3 * kEmbeddingDim);
    tensor fusedBiases = readValues(prefix + ".attn.c_attn.bias.txt", 3 * kEmbeddingDim);

    layer.qWeights.assign(fusedWeights.begin(), fusedWeights.begin() + block);
    layer.kWeights.assign(fusedWeights.begin() + block, fusedWeights.begin() + 2 * block);
    layer.vWeights.assign(fusedWeights.begin() + 2 * block, fusedWeights.end());
    layer.qBiases.assign(fusedBiases.begin(), fusedBiases.begin() + kEmbeddingDim);
    layer.kBiases.assign(fusedBiases.begin() + kEmbeddingDim, fusedBiases.begin() + 2 * kEmbeddingDim);
    layer.vBiases.assign(fusedBiases.begin() + 2 * kEmbeddingDim, fusedBiases.end());

    layer.oWeights = loadMatrix(prefix + ".attn.c_proj.weight.txt", kEmbeddingDim, kEmbeddingDim);
    layer.oBiases = readValues(prefix + ".attn.c_proj.bias.txt", kEmbeddingDim);
    layer.l1Weights = loadMatrix(prefix + ".mlp.c_fc.weight.txt", kEmbeddingDim, kMlpDim);
    layer.l1Biases = readValues(prefix + ".mlp.c_fc.bias.txt", kMlpDim);
    layer.l2Weights = loadMatrix(prefix + ".mlp.c_proj.weight.txt", kMlpDim, kEmbeddingDim);
...
GptWeights loadWeights(const std::string& root = "../weights") {
    GptWeights weights;
    weights.embeddingWeights = readValues(root + "/transformer.wte.weight.txt",
                                          static_cast<std::size_t>(kVocabularySize) * kEmbeddingDim);
    weights.positionalEmbeddingWeights = readValues(root + "/transformer.wpe.weight.txt",
                                                     static_cast<std::size_t>(kMaxPositions) * kEmbeddingDim);
    weights.finalNormWeights = readValues(root + "/transformer.ln_f.weight.txt", kEmbeddingDim);
    weights.finalNormBiases = readValues(root + "/transformer.ln_f.bias.txt", kEmbeddingDim);
...
float gelu(float value) {   // GPT-2 "gelu_new" tanh approximation
    ...
    return 0.5f * value * (1.0f + std::tanh(kSqrt2OverPi * (value + 0.044715f * value * value * value)));
}
...
tensor softmax(std::span<const float> input) {
    ...
    for (std::size_t i = 0; i < input.size(); ++i) result[i] = std::exp(input[i] - maximum);
...
tensor forwardPass(std::span<const float> input, std::span<const float> weights,
                   std::span<const float> biases, bool useGelu = false) {
    ...
    for (std::size_t out = 0; out < outputSize; ++out) {
        ...
        for (std::size_t in = 0; in < input.size(); ++in) sum += input[in] * row[in];
        output[out] = useGelu ? gelu(sum) : sum;
    }
    return output;
}

tensor layerNorm(std::span<const float> input, std::span<const float> weights,
                 std::span<const float> biases) {
    assert(input.size() == weights.size() && input.size() == biases.size());
    const float mean = std::accumulate(input.begin(), input.end(), 0.0f) / input.size();
    float variance = 0.0f;
...
tensor addVectors(std::span<const float> left, std::span<const float> right) {
    assert(left.size() == right.size());
    tensor result(left.begin(), left.end());
    for (std::size_t i = 0; i < result.size(); ++i) result[i] += right[i];
    return result;
}
...
struct KvCache {
    ...
    KvCache() : keys(kLayers), values(kLayers) {
        for (int layer = 0; layer < kLayers; ++layer) {
            keys[layer].reserve(static_cast<std::size_t>(kMaxPositions) * kEmbeddingDim);
            values[layer].reserve(static_cast<std::size_t>(kMaxPositions) * kEmbeddingDim);
        }
    }
};

// One transformer layer on a single new token x: x -> x + Attn(LN1(x)) -> + MLP(LN2(.))
// Attention runs against the cached K/V from all previous tokens (causal, via the cache order).
tensor transformerBlockStep(const TransformerInput& layer, std::span<const float> x,
                            KvCache& cache, int layerIndex) {
    ...
    for (int head = 0; head < kHeads; ++head) {
        ...
        for (int t = 0; t < count; ++t) {
            const float* key = cachedKeys.data() + static_cast<std::size_t>(t) * kEmbeddingDim + headOffset;
            scores[t] = std::inner_product(q.begin() + headOffset, q.begin() + headOffset + kHeadDim,
                                           key, 0.0f) * scale;
        }
        tensor probabilities = softmax(scores);
        for (int t = 0; t < count; ++t) {
            const float* value = cachedValues.data() + static_cast<std::size_t>(t) * kEmbeddingDim + headOffset;
            for (int d = 0; d < kHeadDim; ++d) attended[headOffset + d] += probabilities[t] * value[d];
        }
    }
...
// Runs one token through the full model. computeLogits=false skips the final layernorm and
// the 50257-way logits (used for prompt tokens other than the last one, to save work).
tensor gptStep(const GptWeights& weights, int tokenId, int position, KvCache& cache,
               bool computeLogits) {
    if (position < 0 || position >= kMaxPositions) {
        throw std::runtime_error("Position out of range (max 1024 tokens)");
    }
    tensor x(kEmbeddingDim);
    const std::size_t tokenBase = static_cast<std::size_t>(tokenId) * kEmbeddingDim;
    const std::size_t positionBase = static_cast<std::size_t>(position) * kEmbeddingDim;
    for (int d = 0; d < kEmbeddingDim; ++d) {
        x[d] = weights.embeddingWeights[tokenBase + d] + weights.positionalEmbeddingWeights[positionBase + d];
    }

    for (int layer = 0; layer < kLayers; ++layer) {
        x = transformerBlockStep(weights.transformerWeights[layer], x, cache, layer);
    }
    if (!computeLogits) return {};

    tensor normalized = layerNorm(x, weights.finalNormWeights, weights.finalNormBiases);
    tensor logits(kVocabularySize);   // tied lm_head: logits = normalized . wte[token]
    for (int token = 0; token < kVocabularySize; ++token) {
        auto wordEmbedding = std::span<const float>(weights.embeddingWeights)
            .subspan(static_cast<std::size_t>(token) * kEmbeddingDim, kEmbeddingDim);
        logits[token] = std::inner_product(normalized.begin(), normalized.end(), wordEmbedding.begin(), 0.0f);
    }
    return logits;
}
...
// HF-style repetition penalty: logits for tokens already seen are pulled toward zero
// (positive / penalty, negative * penalty). penalty <= 1.0 is a no-op.
void applyRepetitionPenalty(tensor& logits, const std::vector<int>& seenTokens, float penalty) {
...
// Top-k candidates, then a nucleus (top-p) cutoff on that sorted set, then sample.
int sampleToken(const tensor& logits, float temperature, int topK, float topP, std::mt19937& rng) {
    ...
    // indices[0..topK) are sorted descending by logit, so probabilities[] below is too.

    tensor top(topK);
    for (int i = 0; i < topK; ++i) top[i] = logits[indices[i]] / temperature;
...
std::unordered_map<std::string, int> mergeRanks;         // "left right" -> rank (lower merges first)
...
std::vector<char32_t> utf8ToCodepoints(const std::string& text) {
    ...
    for (std::size_t i = 0; i < text.size();) {
        ...
        const int length = first < 0x80 ? 1 : (first >> 5) == 0x6 ? 2 : (first >> 4) == 0xE ? 3 : 4;
...
// GPT-2's bytes_to_unicode(): printable bytes map to themselves, the rest to a spare range.
void buildByteTables() {
    ...
    int extra = 0;
    for (int b = 0; b < 256; ++b) {
        const char32_t cp = printable[b] ? static_cast<char32_t>(b) : static_cast<char32_t>(256 + extra++);
...
bool isLetterByte(unsigned char c) {   // non-ASCII bytes are treated as letters (\p{L} approximation)
    return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c >= 0x80;
}

// Manual version of GPT-2's pre-tokenizer regex (std::regex has no \p{L} support):
...
std::vector<std::string> preTokenize(const std::string& text) {
    ...
    auto byteAt = [&](std::size_t i) { return static_cast<unsigned char>(text[i]); };
    auto runEnd = [&](std::size_t start) {
        std::size_t end = start;
        const unsigned char first = byteAt(start);
        if (isLetterByte(first)) {
            while (end < n && isLetterByte(byteAt(end))) ++end;
        } else if (isDigitByte(first)) {
            while (end < n && isDigitByte(byteAt(end))) ++end;
        } else {
            while (end < n && !isSpaceByte(byteAt(end)) && !isLetterByte(byteAt(end)) &&
                   !isDigitByte(byteAt(end))) ++end;
        }
        return end;
    };
    ...
    while (i < n) {
        ...
        if (text[i] == ' ' && i + 1 < n && !isSpaceByte(byteAt(i + 1))) {
            ...
            // whitespace run: \s+(?!\S) keeps the last whitespace char for the next token
            std::size_t end = i;
            while (end < n && isSpaceByte(byteAt(end))) ++end;
            if (end < n && end - i > 1) --end;
            pieces.push_back(text.substr(i, end - i));
            i = end;
        }
    }
...
std::vector<int> bpeEncode(const std::string& piece) {
    ...
    for (unsigned char byte : piece) symbols.push_back(byteToUnicode[byte]);
    ...
    while (symbols.size() > 1) {
        ...
        if (bestIndex == symbols.size()) break;
    ...
    for (const std::string& symbol : symbols) {
        ...
        if (it == tokenToId.end()) throw std::runtime_error("Token not in vocab: " + symbol);
...
std::vector<int> encode(const std::string& text) {
    ...
    for (const std::string& piece : preTokenize(text)) {
        for (int id : bpeEncode(piece)) ids.push_back(id);
    }
    return ids;
}

// Number of trailing bytes forming an incomplete multi-byte UTF-8 sequence (0 = complete).
// Used to avoid printing a character split across two token boundaries mid-stream.
...
// vocab.json is a flat {"token": id, ...} object; parsed manually to avoid a JSON dependency.
void loadVocab(const std::string& path) {
    std::ifstream file(path, std::ios::binary);
    if (!file) throw std::runtime_error("Could not open vocab file: " + path);
    ...
    std::size_t pos = 0;
    auto skipSpaces = [&] { while (pos < text.size() && isSpaceByte(static_cast<unsigned char>(text[pos]))) ++pos; };
    auto expect = [&](char c) {
        skipSpaces();
        if (pos >= text.size() || text[pos] != c) {
            throw std::runtime_error(std::string("vocab.json parse error, expected '") + c + "'");
        }
        ++pos;
    };
    auto parseString = [&]() {
        ...
        while (pos < text.size() && text[pos] != '"') {
            ...
            switch (escape) {
                ...
                case 'u':
                    out += codepointToUtf8(static_cast<char32_t>(std::stoul(text.substr(pos, 4), nullptr, 16)));
                    pos += 4;
                    break;
                default: throw std::runtime_error("vocab.json: bad escape");
            }
        }
        ++pos;
        return out;
    ...
    while (pos < text.size() && text[pos] != '}') {
        ...
        skipSpaces();
        int id = 0;
        while (pos < text.size() && isDigitByte(static_cast<unsigned char>(text[pos]))) {
            id = id * 10 + (text[pos++] - '0');
        }
        if (id < 0 || id >= kVocabularySize) throw std::runtime_error("vocab.json: id out of range");
        gpt2Tokens[id] = token;
...
// merges.txt: each line is "left right"; line number is the rank. First line may be a header.
void loadMerges(const std::string& path) {
    std::ifstream file(path);
    if (!file) throw std::runtime_error("Could not open merges file: " + path);
    mergeRanks.clear();
    std::string line;
    int rank = 0;
    bool firstLine = true;
...
std::string generate(const GptWeights& weights, const std::string& prompt, int maxNewTokens,
                     float temperature, int topK, float topP, float repetitionPenalty) {
    std::vector<int> ids = encode(prompt);
    if (ids.empty()) ids.push_back(kEndOfText);
    ...
    for (std::size_t i = 0; i < ids.size(); ++i) {
        logits = gptStep(weights, ids[i], static_cast<int>(i), cache, i + 1 == ids.size());
    }

    std::string generated;
    std::string pendingUtf8;   // holds a multi-byte UTF-8 char split across two tokens
    std::cout << prompt << std::flush;
    for (int step = 0; step < maxNewTokens; ++step) {
        applyRepetitionPenalty(logits, ids, repetitionPenalty);
        const int next = temperature <= 0.0f ? argmaxToken(logits)
                                             : sampleToken(logits, temperature, topK, topP, rng);
        if (next == kEndOfText) break;
        ids.push_back(next);
        ...
        if (step + 1 == maxNewTokens || ids.size() >= static_cast<std::size_t>(kMaxPositions)) break;
        logits = gptStep(weights, next, static_cast<int>(ids.size()) - 1, cache, true);
    }
...
int main(int argc, char** argv) {
    try {
        ...
        const int maxNewTokens = positional.size() > 1 ? std::stoi(positional[1]) : 20;
        const float temperature = positional.size() > 2 ? std::stof(positional[2]) : 0.8f;
        const int topK = positional.size() > 3 ? std::stoi(positional[3]) : 40;
        const float topP = positional.size() > 4 ? std::stof(positional[4]) : 0.9f;
        const float repetitionPenalty = positional.size() > 5 ? std::stof(positional[5]) : 1.2f;
        ...
        GptWeights weights = loadWeights(root);   // loaded once, reused for every prompt below
        std::cerr << "Loaded GPT-2 weights: " << weights.transformerWeights.size() << " layers\n";

        if (!positional.empty()) {
            generate(weights, positional[0], maxNewTokens, temperature, topK, topP, repetitionPenalty);
        }

        std::cout << "\nInteractive mode — leave any field blank to keep the current default.\n"
        ...
        while (true) {
            ...
            if (prompt.empty() || prompt == "exit" || prompt == "quit") break;

            const int   roundTokens           = readIntOrDefault  ("tokens",           maxNewTokens);
            const float roundTemperature      = readFloatOrDefault("temperature",       temperature);
            const int   roundTopK             = readIntOrDefault  ("topK",              topK);
            const float roundTopP             = readFloatOrDefault("topP",              topP);
            const float roundRepetitionPenalty= readFloatOrDefault("repetitionPenalty", repetitionPenalty);

            generate(weights, prompt, roundTokens, roundTemperature, roundTopK, roundTopP, roundRepetitionPenalty);
        }
